...
class SmartThingsIntegration:
    ...
    def _register_event_handlers(self):
        ...
        async def on_connect():
            ...
            # REBOOT SURVIVAL FIX: Force fresh config reload from disk
        ...
        async def on_subscribe_entities(entity_ids: List[str]):
            ...
            # REBOOT SURVIVAL FIX: Check if entities exist, recreate if missing
            if st_entities:
                missing_entities = []
                ...
                if missing_entities:
                    _LOG.warning(f"Missing {len(missing_entities)} entities after reboot - recreating all")
                    if await self._load_existing_configuration():
    ...
    async def _load_existing_configuration(self) -> bool:
        ...
        try:
            # REBOOT SURVIVAL FIX: Force fresh config load from filesystem
    ...
    async def _create_placeholder_entities(self):
        ...
        try:
            ...
            # REBOOT SURVIVAL FIX: Clear any existing entities and create fresh ones
